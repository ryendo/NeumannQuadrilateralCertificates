...
current_fields={'mass_matrix_lower','lambda3_lower','lambda3_minus_3pi2_over2_lower'};
current_schema=all(ismember(current_fields,T.Properties.VariableNames));
...
verified=legacy_verified && current_schema && minMass>0 && minL3>16 && minL3Gap>0;
summary=struct('verified',verified, ...
    'legacy_certificate_verified',legacy_verified, ...
    'current_algorithm_fields_present',current_schema, ...
...
if summary.verified
    fprintf(['VERIFIED: S(t,e)>0, M>0, index 1, and lambda_3>16 are ' ...
        'certified on all saved boxes.\n']);
elseif summary.legacy_certificate_verified && ~summary.current_algorithm_fields_present
