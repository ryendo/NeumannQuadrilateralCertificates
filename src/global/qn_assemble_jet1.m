...
for iu=1:20
    ...
    for iv=1:20
        ...
        if inf(J.v)<=0, error('qn:Jacobian','Jet assembly Jacobian crosses zero.'); end
        gvv=jadd(jmul(Xv,Xv),jmul(Yv,Yv));
        guv=jadd(jmul(Xu,Xv),jmul(Yu,Yv));
        guu=jadd(jmul(Xu,Xu),jmul(Yu,Yu));
        ...
        for q=1:5
            ...
            phi{q}=jmul(cx,cy);
            psiX=jscale(jmul(sx,cy),-mm*PI);
            psiY=jscale(jmul(cx,sy),-nn*PI);
            phiu{q}=jadd(jmul(psiX,Xu),jmul(psiY,Yu));
            phiv{q}=jadd(jmul(psiX,Xv),jmul(psiY,Yv));
        end
        for i=1:5
            means{i}=jadd(means{i},jscale(jmul(phi{i},J),W));
            for j=i:5
                t1=jmul(gvv,jmul(phiu{i},phiu{j}));
                t2=jmul(guv,jadd(jmul(phiu{i},phiv{j}),jmul(phiv{i},phiu{j})));
                t3=jmul(guu,jmul(phiv{i},phiv{j}));
                K{i,j}=jadd(K{i,j},jscale(jdiv(jadd(jsub(t1,t2),t3),J),W));
...
function z=jdiv(x,y), z=jet(x.v/y.v,(x.g*y.v-x.v*y.g)/(y.v^2)); end
