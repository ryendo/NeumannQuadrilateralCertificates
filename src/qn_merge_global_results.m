...
recorded_provenance_valid=false;
current_source_matches_recorded=false;
if current_schema_present
    [recorded_provenance_valid,current_source_matches_recorded]= ...
        validate_current_provenance(items);
end
current_provenance_valid=recorded_provenance_valid && ...
    current_source_matches_recorded;
...
result.recorded_provenance_valid=recorded_provenance_valid;
result.current_source_matches_recorded=current_source_matches_recorded;
...
result.complete=current_schema_present && current_worker_bounds_valid && ...
    recorded_provenance_valid && current_initial_assignment_valid && ...
...
function [recorded_valid,current_match]=validate_current_provenance(items)
expected_veigs_commit='6556d39a0d9819bb172d232062b698aa76e420f6';
first=items{1};
recorded_valid=ischar(first.source_commit) && ...
...
% Authenticate the computation against the executable tree at its recorded
% commit.  Equality with the current checkout is useful provenance evidence,
% but is not needed for the mathematical completion of a historical run.
recorded_valid=strcmp(first.source_code_sha256,recorded_source_sha256);
if ~recorded_valid, return; end
try
    [~,current_dirty]=qn_git_state(repo_root,{'results/'});
    current_source_sha256=qn_source_code_sha256(repo_root);
    current_match=~current_dirty && ...
        strcmp(first.source_code_sha256,current_source_sha256);
catch
    % Current-checkout inspection is diagnostic and cannot invalidate the
    % authenticated historical computation.
    current_match=false;
end
recorded_valid=all(cellfun(@(x)strcmp(x.source_commit,first.source_commit) && ...
...
if ~recorded_valid, current_match=false; end
end
