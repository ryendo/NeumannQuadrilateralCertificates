...
[~,M,area]=qn_km_enclosure(center,hw);
interiorValid=qn_interval_ldl_pd(M) && inf(area)>0;
...
[~,Mb,Ab]=qn_km_enclosure(boundaryCenter,boundaryHw);
boundaryValid=qn_interval_ldl_pd(Mb) && inf(Ab)>0;
...
if local.current_algorithm_fields_present
    assert(local.verified,'Saved current-schema local certificate is incomplete.');
else
    assert(local.legacy_certificate_verified && ~local.verified, ...
        'Checked-in legacy local results are incomplete or misclassified as current.');
end
report=struct('constants',struct('rho_local',mid(C.rho_local),'rho_seam',mid(C.rho_seam)), ...
    ...
    'local_saved_results_current_schema',local.current_algorithm_fields_present);
