...
`671ddd9f6fb34dbb74b4474a3bdf71d11c53b766`, as recorded in
`results/local/RUN_PROVENANCE.txt`. The checked-in run has 32 result files,
32 completion markers, and 32 metadata files. The verifier checks the exact
box-ID cover, every recorded bound, the source/dependency provenance, and the
SHA-256 bindings from each metadata file to its CSV and completion marker.
...
accepted boxes:                                       114,627
discarded boxes:                                      15,623
unresolved boxes:                                     0
bisections:                                           130,234
maximum subdivision depth:                            30
inf q on accepted boxes:                              0.8273243052571837
inf lambda_1 on accepted boxes:                       3.4247176582671237
...
| Algorithm 1, `alg:single-box-certificate` | Conditions (34)--(35), equations (36), (38)--(39), and $S>0$ | `src/qn_single_box_certificate.m` | Table 3, `tab:local-second-order-certificate`; saved run at source commit `671ddd9` |
| Proposition 6.1, `p:box-bound`; equations (43)--(45), `eq:global-box-notation`--`eq:global-accepted-box-conclusion` | The five conditions in (44) and the conclusion in (45) | `src/qn_km_enclosure.m`, `src/qn_interval_ldl_pd.m`, `src/qn_certify_box.m`, `src/qn_global_code_test.m`, `src/qn_veigs_indices.m` | `tests/qn_smoke_test.m`; `results/global/` |
| Equations (46)--(47), `eq:global-discard-test`, `eq:global-bisection` | Discard and outward-covering bisection rules | `src/qn_box_inside_ball.m`, `src/qn_box_outside_pk.m`, `src/qn_bisect_box.m` | `tests/qn_smoke_test.m`; `results/global/` |
| Equation (48), `eq:global-completion-test` | $\mathcal U=\varnothing$, positive certified minima, and exact initial assignment | `src/qn_global_certified_cover.m`, `src/qn_merge_global_results.m` | `results/global/summary.json` |
| Equation (49), `eq:global-final-cover`; Theorem 6.2, `t:box-cover-terminates` | Finite cover of $\Omega_{\mathrm{II}}$ | `src/qn_global_certified_cover.m` | current run at source commit `671ddd9`; aggregate summaries in `results/global/` |
